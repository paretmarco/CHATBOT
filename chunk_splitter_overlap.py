...
output_dir = "C:/D/documenti/AI/program24/chunker_split_exercises"
...
for input_filename in os.listdir(input_dir):
    ...
    for sentence in sentences:
        ...
        if current_token_count + sentence_token_count <= target_token_count + extra_tokens:
            current_chunk.append(sentence)
            current_token_count += sentence_token_count
        else:
            chunks.append(' '.join(current_chunk))
            current_chunk = current_chunk[-overlap_token_count:] + [sentence]
            current_token_count = len(word_tokenize(' '.join(current_chunk)))
            extra_tokens = 0

        if current_token_count >= target_token_count:
            extra_tokens += sentence_token_count
