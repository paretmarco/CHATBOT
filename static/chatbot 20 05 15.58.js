    ...
    if (response.status === 200) {
      ...
      try {
        while (true) {
          ...
          if (message.choices[0].finish_reason && message.choices[0].delta.content) {  // Check if content exists
            $("#chatbot_response").html(message.choices[0].delta.content);
            $("#edited_answer").val(message.choices[0].delta.content);
          }
        }
      } catch (err) {
